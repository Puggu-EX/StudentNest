...
export default function Enrollment() {
  ...
  const handleMarkToDrop = (enr) => {
    ...
    setAddCart((prev) => prev.filter((s) => sectionKey(s) !== sectionKey(enr)));
  };

  const removeFromAddCart = (sec) => {
    setAddCart((prev) => prev.filter((s) => sectionKey(s) !== sectionKey(sec)));
  };
              ...
              <tbody>
                {loadingTable ? (
                  ...
                  enrollments.map((enr) => {
                    const id = sectionKey(enr);
                    const courseName =
                      enr.className ?? enr.courseName ?? enr.course_name ?? "—";
                    const dept =
