...
export default function Register() {
  ...
  const validate = () => {
    ...
    if (roleValue === "student") {
      ...
      if (!isYear(enrollmentYear))
        return "Enter a valid enrollment year (e.g., 2025).";
    }

    if (roleValue === "chair") {
      if (!building.trim()) return "Building is required for Department Chair.";
      if (!roomNumber.trim())
        return "Room Number is required for Department Chair.";
      if (!isEmail(contactEmail))
        return "Enter a valid contact email for Department Chair.";
      if (!isPhone(phone))
        return "Enter a valid contact phone number for Department Chair.";
    }
    return null;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        navigate(
          `/account-failure?status=${encodeURIComponent(
            String(res.status)
      ...
      navigate(
        `/account-failure?status=network&msg=${encodeURIComponent(
  ...
  return (
    <div className="register-page">
      <form className="register-card" onSubmit={handleSubmit} noValidate>
        <h1 className="title">Create Your StudentNest Account!</h1>
        {/* Register using socials */}
        {/* <p className="subtitle">Or register using</p>
        <div className="social-icons">
          <a href="#" className="icon google">
            G
          </a>
          <a href="#" className="icon microsoft">
            M
          </a>
        </div> */}
        ...
        {role === "student" && (
          <>
            <select name="major_choices" onChange={(e) => setMajor(e.target.value)}>
